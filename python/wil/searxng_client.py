...
class SearXNGClient:
    ...
    async def search_multi(self, query: str, categories: List[str], limit_per_category: int = 5) -> List[Dict[str, Any]]:
        ...
        # Concurrent execution with strict 2.5 seconds timeout
        try:
            results_lists = await asyncio.wait_for(asyncio.gather(*tasks, return_exceptions=True), timeout=2.5)
        except asyncio.TimeoutError:
            LOGGER.warning("SearXNG search_multi timed out after 2.5 seconds.")
